    ...
    RestClient.get('articles/with-comments?limit=3', 
      function(response) {
        ...
        articles.slice(0, 3).forEach(function(article) {
          html += `<div class="nt-item">${Utils.escapeHtml(article.title)}</div>`;
        });
          ...
          html += `
            <div class="col-lg-3 col-sm-6 p-0">
              <div class="soccer-item set-bg" data-setbg="${imageUrl}">
                ...
                <div class="si-text">
                  <h5><a href="#article?id=${article.article_id}">${Utils.escapeHtml(article.title)}</a></h5>
                  <ul>
        ...
        html += `
          <div class="col-md-6">
            <div class="news-item left-news">
              ...
              <div class="ni-text">
                <h4><a href="#article?id=${bigArticle.article_id}">${Utils.escapeHtml(bigArticle.title)}</a></h4>
                <ul>
                  <li><i class="fa fa-calendar"></i> ${bigDate}</li>
                  <li><i class="fa fa-edit"></i> ${bigCommentCount} Comments</li>
                </ul>
                <p>${Utils.escapeHtml(excerpt)}</p>
              </div>
            </div>
          </div>
        `;
